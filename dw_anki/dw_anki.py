...
def reduceImageSize(path):
    ...
    res = subprocess.run(
        ["convert", "-resize", "25%", backupFilePath, path],
...
def buildAnkiFromURL(cards, vocabURL):
    ...
    for row in vocab_rows:
        ...
        if en == None or de == None:
            log.info(
                "Could not find english and/or german word from row. Possibly the last row is empty div."
            )
            continue
        log.debug("de = " + de)
        log.debug("en = " + en)
...
def storeCards(cards):
    for card in sorted(cards.values(), key=operator.attrgetter("cardNumber")):
        req = addNoteJSON(card.deck, card.tags, card.getEnglish(), card.getGerman())
        try:
            res = invoke(req)
            if card.hasImage:
